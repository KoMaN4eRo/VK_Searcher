...
using VkSearcher.JsonModel.UsersLike;
using VkSearcher.JsonModel.UsersWall;
...
namespace VkSearcher.Code
{
    public class VkUserInfo
    {
        ...
        //public Dictionary<int, WallItems> GetWallInfoReal(List<FriendsItems> list) // Возвращает список айдишники записей на стене у друзей целевого пользователя ПОМЕНЯТЬ ТИП ВОЗВРАЩЯЕМОГО РЕЗУЛЬТАТА
        //{ 
        ...
        public List<UsersPhoto> GetUsesLike_Photo(List<PhotoInfo> photoInfo, string friendsId, string targetUserId)
        {
            ...
            LikesData response = null;
            List<UsersPhoto> finalResponse = new List<UsersPhoto>();
                    ...
                    else
                    {
                        foreach (Like like in response.like)
                        {
                            foreach (User user in response.like[loopCounter].items)
                            {
                                ...
                                {
                                    finalResponse.Add(new UsersPhoto(photoInfo[idCounter].owner_id, photoInfo[idCounter].id, photoInfo[idCounter].album_id));// Дичь с цыклом
                                }
                                else { continue; }
                            }
                            idCounter++;
                ...
                else
                {
                    ...
                    foreach (Like like in response.like)
                    {
                        foreach (User user in response.like[loopCounter].items)
                        {
                            if (friendsId == user.id)
                            {
                                finalResponse.Add(new UsersPhoto(photoInfo[idCounter].owner_id, photoInfo[idCounter].id, photoInfo[idCounter].album_id));
        ...
        public string GroupsInfo(string UserId, string count)
        {
            string access_token = "e7c520cc53cf2bcb2a675cc8c767de065abebfe15f0913b2b702659124b8409441707b4dac8a8c4e58eaf";
            string term = String.Format("https://api.vk.com/method/groups.get?user_id={0}&count={1}&access_token={2}&v=5.50", UserId, count,access_token);
            var response = Load(term);// После 2 вызова 3 элемент фолсе ИСПРАВИТЬ
            return response;
